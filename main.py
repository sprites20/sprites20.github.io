...
def handle_connect():
    ...
    USER_DB_DIR = "users"  # Directory to store per-user databases

    # Ensure the users directory exists
    os.makedirs(USER_DB_DIR, exist_ok=True)
    
    # Define the user's database path
...
def handle_get_conversation_ids(data):
    ...
    # Check if conversation IDs are cached
    if user_id in conversation_cache:
        emit('conversation_ids', {'conversationIds': conversation_cache[user_id]})
        return
    ...
    # Store in cache
    conversation_cache[user_id] = conversation_ids
...
class DraggableLabelApp(MDApp):
    ...
    def add_message(self, isVisionModel, role, user_text=None, image=None, user_id=None, conversation_id=None):
        ...
        # Check if the system message is already added in this conversation
        ...
        if conversation_id not in self.past_messages[user_id]:
            ...
            self.past_messages[user_id][conversation_id].append(system_message)
        ...
        if isVisionModel:
            if image is not None:
                ...
                self.past_messages[user_id][conversation_id].append(message)
            else:
        ...
        # Store in DuckDB (assistant/user message)
        conns[user_id].execute(
            "INSERT INTO messages (conversation_id, role, content) VALUES (?, ?, ?)",
            [conversation_id, message["role"], json.dumps(message["content"])]
        )
        
    def load_past_messages(self, user_id):
        """Load past messages for a specific user from DuckDB into cache"""
        results = conns[user_id].execute("SELECT conversation_id, role, content FROM messages ORDER BY timestamp").fetchall()
